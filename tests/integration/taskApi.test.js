...
describe('Task API Integration Tests', () => {
  ...
  describe('Task Sorting Integration', () => {
    test('should integrate sorting with service and persist sort preferences', () => {
      // Ajouter des tâches avec différents timestamps
      taskService.addTask('Zebra task');
      setTimeout(() => taskService.addTask('Alpha task'), 10); 
      setTimeout(() => taskService.addTask('Beta task'), 20);
...
function resetLocalStorage() {
  Object.defineProperty(window, 'localStorage', {
    ...
    test('should handle invalid import data', () => {
      const result1 = taskService.importTasks('invalid json');
      const result2 = taskService.importTasks('{"tasks": "not-array"}');
      
      expect(result1.success).toBe(false);
      expect(result1.error).toBe('Invalid JSON format');
      
      expect(result2.success).toBe(false);
      expect(result2.error).toBe('Invalid data format');
    });
  });
